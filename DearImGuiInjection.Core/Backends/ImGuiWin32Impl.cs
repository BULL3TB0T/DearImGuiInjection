...
public static unsafe class ImGuiWin32Impl
{
    ...
    private static bool ImGui_ImplWin32_InitEx(void* windowHandle, bool platform_has_own_dc)
    {
        ...
        main_viewport.PlatformHandle = main_viewport.PlatformHandleRaw = windowHandle;
    ...
    static void ImGui_ImplWin32_ProcessKeyEventsWorkarounds()
    {
        ...
        if (ImGui.IsKeyDown(ImGuiKey.LeftShift) && !IsVkDown(VirtualKey.VK_LSHIFT))
            ImGui_ImplWin32_AddKeyEvent(ImGuiKey.LeftShift, false, VirtualKey.VK_LSHIFT);
        if (ImGui.IsKeyDown(ImGuiKey.RightShift) && !IsVkDown(VirtualKey.VK_RSHIFT))
            ImGui_ImplWin32_AddKeyEvent(ImGuiKey.RightShift, false, VirtualKey.VK_RSHIFT);

        // Sometimes WM_KEYUP for Win key is not passed down to the app (e.g. for Win+V on some setups, according to GLFW).
        if (ImGui.IsKeyDown(ImGuiKey.LeftSuper) && !IsVkDown(VirtualKey.VK_LWIN))
            ImGui_ImplWin32_AddKeyEvent(ImGuiKey.LeftSuper, false, VirtualKey.VK_LWIN);
        if (ImGui.IsKeyDown(ImGuiKey.RightSuper) && !IsVkDown(VirtualKey.VK_RWIN))
            ImGui_ImplWin32_AddKeyEvent(ImGuiKey.RightSuper, false, VirtualKey.VK_RWIN);
    }
    ...
    public static void ImGui_ImplWin32_UpdateKeyModifiers()
    {
        ...
        io.AddKeyEvent(ImGuiKey.ModSuper, IsVkDown(VirtualKey.VK_APPS));
    }
    ...
    static void ImGui_ImplWin32_UpdateGamepads()
    {
        ...
        void MAP_BUTTON(ImGuiKey key, XInputGamepad button) => io.AddKeyEvent(key, (gamepad.wButtons & (ushort)button) != 0);
        void MAP_ANALOG(ImGuiKey key, int value, int v0, int v1) { float vn = (value - v0) / (float)(v1 - v0); io.AddKeyAnalogEvent(key, vn > 0.10f, IM_SATURATE(vn)); }
    ...
    public const VirtualKey IM_VK_KEYPAD_ENTER = (VirtualKey)((int)VirtualKey.VK_RETURN + 256);
        ...
        switch (msg)
        {
            ...
            case WindowMessage.WM_SYSKEYUP:
                {
                    ...
                    if ((int)wParam < 256)
                    {
                        ...
                        bool isEnter = (VirtualKey)wParam == VirtualKey.VK_RETURN;
                        bool hasExtendedKeyFlag = (HIWORD(lParam) & (ushort)KeyFlag.KF_EXTENDED) != 0;
                        ...
                        if (vk == VirtualKey.VK_SHIFT)
                        {
                            ...
                            if (IsVkDown(VirtualKey.VK_LSHIFT) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(ImGuiKey.LeftShift, is_key_down, VirtualKey.VK_LSHIFT, scancode); }
                            if (IsVkDown(VirtualKey.VK_RSHIFT) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(ImGuiKey.RightShift, is_key_down, VirtualKey.VK_RSHIFT, scancode); }
                        }
                        else if (vk == VirtualKey.VK_CONTROL)
                        {
                            if (IsVkDown(VirtualKey.VK_LCONTROL) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(ImGuiKey.LeftCtrl, is_key_down, VirtualKey.VK_LCONTROL, scancode); }
                            if (IsVkDown(VirtualKey.VK_RCONTROL) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(ImGuiKey.RightCtrl, is_key_down, VirtualKey.VK_RCONTROL, scancode); }
                        }
                        else if (vk == VirtualKey.VK_MENU)
                        {
                            if (IsVkDown(VirtualKey.VK_LMENU) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(ImGuiKey.LeftAlt, is_key_down, VirtualKey.VK_LMENU, scancode); }
                            if (IsVkDown(VirtualKey.VK_RMENU) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(ImGuiKey.RightAlt, is_key_down, VirtualKey.VK_RMENU, scancode); }
                        }
                    }
                    return IntPtr.Zero;
                }
            ...
            case WindowMessage.WM_CHAR:
                ...
                else
                {
                    ...
                    Kernel32.MultiByteToWideChar(Kernel32.CP_ACP, Kernel32.MB_PRECOMPOSED, [*(byte*)&wParam ], 1, wideBuffer, wideBufferSize);
